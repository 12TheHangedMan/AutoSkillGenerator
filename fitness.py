...
def calculate_dmg_loss(total_dmg: float, target_hp: int) -> float:
    return (config.TARGET_ROUNDS - target_hp / (total_dmg * 1 + 1e-6)) ** 2


def calculate_cost_loss(total_cost: float, total_dmg: float) -> float:
    return (config.TARGET_DMG_COST_RATIO - total_dmg / (total_cost * 1 + 1e-6)) ** 2


def calculate_fatigue_loss(total_fatigue: float, total_dmg: float) -> float:
    return (config.TARGET_DMG_FATIGUE_RATIO - total_dmg / (total_fatigue * 1 + 1e-6)) ** 2
